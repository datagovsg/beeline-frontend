...
let transformKickstarterData = function (kickstarterRoutes) {
  ...
  for (let kickstarter of kickstarterRoutes) {
    ...
    if (kickstarter.bids && kickstarter.bids.length > 0) {
     var bidsByTier = _.groupBy(kickstarter.bids, (x) => x.priceF)
      kickstarter.notes.tier.map((tier)=>{
        let countCommitted = bidsByTier[tier.price] ? bidsByTier[tier.price].length :0
    ...
    let passExpiryTime = new Date(firstTripDate.getFullYear(), firstTripDate.getMonth()+1, firstTripDate.getDate()).getTime()
...
export default ['$http', 'UserService', '$q',
  ...
  function KickstarterService ($http, UserService, $q, $rootScope, RoutesService, p, DevicePromise) {
    ...
    let bidsCache
    let kickstarterSummary = null, bidsById = null
    let kickstarterRoutesList = null, kickstarterRoutesById = null
