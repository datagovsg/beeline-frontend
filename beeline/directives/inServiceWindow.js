    ...
    return {
      ...
      link: function(scope, element, attributes) {
        ...
        scope.timeout = new SafeInterval(
          () => checkServiceWindow(scope.availableTrips),
          1000 * 60,
          1000
        )
        scope.timeout.start()
        ...
        function checkServiceWindow(trips) {
          if (!trips || trips.length == 0) {
            ...
            let allStopsTime = _.flatten(trips.map(trip => trip.tripStops))
              .map(stop => stop.time)
              .sort()
            if (allStopsTime) {
              // 15 min before 1st stop time
              scope.startTime =
                new Date(allStopsTime[0]).getTime() - 1000 * 60 * 15
              scope.endTime = new Date(
                allStopsTime[allStopsTime.length - 1]
              ).getTime()
              scope.now = Date.now()
              if (isFinite(scope.startTime) && isFinite(scope.endTime)) {
                scope.inServiceWindow =
                  scope.startTime <= scope.now && scope.now <= scope.endTime
              } else {
