...
export default [
  ...
  "OneMapService",
  "UserService",
  function($http, OneMapService, UserService) {
    return {
      restrict: "E",
      transclude: true,
      scope: {
    ...
    {{y | number:4}}, {{x | number:4}}
      ...
      link: async function(scope, elem) {
        ...
        function updateDescription() {
          if (
            scope.geocodeW &&
            scope.geocodeW.GeocodeInfo &&
            scope.geocodeW.GeocodeInfo[0].ROAD
          ) {
            scope.description1 =
              titleCase(scope.geocodeW.GeocodeInfo[0].BLOCK) +
              " " +
              titleCase(scope.geocodeW.GeocodeInfo[0].ROAD)
            scope.description2 = titleCase(
              scope.geocodeW.GeocodeInfo[0].BUILDINGNAME
            )
          } else {
            scope.description1 =
        ...
        scope.$watchGroup(["description1", "description2"], function() {
          if (
            scope.description1 == undefined ||
            scope.description2 == undefined
          ) {
            updateDescription()
          }
        })
